    ...
    <body>
        ...
        <c:if test="${not empty pedidosAttr}">
            <table class="table table-sm offset-md-2" style="width: 75%;">
                ...
                <tbody>
                    <c:forEach items="${pedidosAttr}" var="pedidos">
                        <tr>
                            <th> <c:out value="${pedidos.getId()}"/></th>
                            <td ><c:out value="${pedidos.getIdCliente()}"/></td>
                            <td><c:out value="${pedidos.getStatusCompra()}"/></td>
                            <td> <c:out value="${pedidos.getValorTotal()}"/></td>
                            <td><c:out value="${pedidos.getDataVenda()}"/></td>

                            <td><a data-method="get" href="${pageContext.request.contextPath}/estoquista/atualizar-status?id=${pedidos.getId()}" class="btn btn-primary mb-1" >Atualizar</a></td>
